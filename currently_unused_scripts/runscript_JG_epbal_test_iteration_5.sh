...
###build up CaseNames, RunDirs, Archive Dirs, etc.
    ...
    CaseName=$JG_CaseName_Root"$t"_epbal_test
    PreviousBGCaseName="$BG_CaseName_Root""$tm1"
    JG_t_RunDir=/glade/scratch/jfyke/$CaseName/run
    BG_tm1_ArchiveDir=/glade/scratch/sprice/$PreviousBGCaseName/run
...
###configure POP
    ...
    echo ladjust_precip=.true. > user_nl_pop
...
###specify custom run directory scratch location
    ./xmlchange RUNDIR="/glade/scratch/sprice/$CaseName/run"
