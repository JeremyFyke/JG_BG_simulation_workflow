...
###build up CaseNames, RunDirs, Archive Dirs, etc.
    ...
    CaseName=$JG_CaseName_Root"$t"_test_multiyear_submission
...
###concatenate monthly forcing files to expected location
    OutDir=/glade/scratch/jfyke/archive/BG_iteration_2/cpl/hist
    if [ ! -d $OutDir ]; then
       echo 'Error: directory where you want to put monthly-concatenated cpl history forcing files:'
       echo $OutDir
	...
	for m in `seq -f '%02g' 1 12`; do
	   for ftype in ha2x1hi ha2x1h ha2x3h ha2x1d; do       
	      fname_out=$OutDir/$PreviousBGCaseName.cpl.$ftype.$yr-$m.nc
	      if [ -f $fname_out ]; then
...
###configure datm streams manually (mostly this is to force a no-read of topo and presaero, which I can't easily disable
    ...
    echo   'streams = "datm.streams.txt.CPLHIST_JG.Solar 1 1 13", "datm.streams.txt.CPLHIST_JG.Winds 1 1 13", "datm.streams.txt.CPLHIST_JG.Precip 1 1 13", "datm.streams.txt.CPLHIST_JG.nonSolarNonPrecip 1 1 13", "datm.streams.txt.CPLHIST_JG.Daily 1 1 13"' >> user_nl_datm
